...
export class WClockComponent implements OnChanges {
  ...
  private setupUI() {
    console.log("DEBUG: user time: " + JSON.stringify(this.userTime));
    ...
    switch (this.currentView) {
      ...
      case CLOCK_TYPE.MINUTES:
        ...
        for (let i = this.minuteStep; i < 60; i += this.minuteStep) {
  ...
  public getPointerStyle() {
    ...
    if (this.currentView === CLOCK_TYPE.HOURS && this.userTime.hour <= 12 && this.userTime.format === 24) {
  ...
  public getTimeValueClass(step: number, index: number) {
    ...
    const stepDeg = this.currentView === CLOCK_TYPE.HOURS ? 360 / 12 : 360 / (60/this.minuteStep);
  ...
  public changeTimeValue(step: number) {
    ...
    if (this.currentView === CLOCK_TYPE.HOURS) {
      ...
      this.selectedTimePart = this.userTime.minute;
  ...
  public displayStep(step: number) {
    ...
    // if (this.currentView === CLOCK_TYPE.MINUTES && step % this.minuteStep !== 0) return '';
